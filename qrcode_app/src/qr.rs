...
pub fn generate_qr(link: &str, args: &Args) -> Result<String, String> {
    ...
    let fg = parse_hex_color(&args.fg)
        .map_err(|e| format!("Couleur de premier plan invalide: {e}"))?;
    let bg = parse_hex_color(&args.bg)
        .map_err(|e| format!("Couleur de fond invalide: {e}"))?;
    ...
    let mut colored: ImageBuffer<Rgba<u8>, Vec<u8>> = if let Some(bg_img) = background {
        bg_img
    } else {
